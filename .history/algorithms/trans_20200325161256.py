...
def get_height_in_cm(height):
    height_in_cm = round(float(height.replace('-','.')) * 0.4535924, 1)
    return height_in_cm

def get_weight_in_cm(weight):
    weight_in_kg = round(float(weight.replace('-','.')) * 0.4535924, 1)
    return get_weight_in_cm
