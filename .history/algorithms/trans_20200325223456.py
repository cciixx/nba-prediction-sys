...
def transform(name, height, weight, pos, stl, reb, ast, fouls, dunk, efgp):
    ...
    reb = reb
    pas = ast
    phy = 0.1*float(fouls) + 0.3*get_height(height) + 0.4*get_weight(weight) + 0.2*drib
    jump = 0.5*float(dunk) + 0.5*float(reb)
    run = 0.2*get_height(height) + 0.3*get_weight(weight) + 0.5*float(drib)
    shoot = float(efgp)
    name = name
    result = json.dumps({"name": name, "stl": float(steal), "reb": float(reb), "drib": drib, "pass": float(pas), "phy": phy, "jump": jump, "run": run, "shoot": shoot})
    return result
